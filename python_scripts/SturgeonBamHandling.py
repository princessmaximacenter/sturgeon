...
class NewBamFileHandler(FileSystemEventHandler):
    def __init__(self, sturgeon_script_path: Path, output: Path, model: Path, freq: int, utils: Path, r_script_path: Path, input: Path, gridion: bool, shutdown_event: threading.Event, live_run: bool) -> None:
        self.iteration = 1
    ...
    def _process_gridion_run(self) -> None:
        ...
        :return: None
        """
        app_log.info("Processing files from gridion run")

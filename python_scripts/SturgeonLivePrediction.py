...
class SturgeonPrediction:
    ...
    def __init__(self, **kwargs):
        # Store all click parameters in attributes
    ...
    def _setup(self):
        """
        Handles output directory, logging initialization, lock creation
        """
        ...
        # Handle results directory and lock file
    ...
    def _wait_for_input_directory(self) -> bool:
        ...
        while not self.shutdown_event.is_set():
            ...
            if self.input.exists():
                self.app_log.info(f"Results directory {input} found, proceeding with sturgeon analysis.")
                return True
            if wait_time % log_interval_time == 0:
                self.app_log.info(f"Waiting for results directory {input} to be created.")
            time.sleep(1)
            wait_time += 1
    ...
    def _file_cleanup_shutdown(self) -> None:
        ...
        try:
            src_files = [f"CNV_plot_iteration_{last_iteration}.pdf", f"confidence_over_time_plot_iteration_{last_iteration}.pdf",
                         f"merged_probes_methyl_calls_general_iteration_{last_iteration}.pdf",
    ...
    def _cleanup(self):
        ...
        try:
            ...
            self.app_log.info(f"Final metadata report save to {metadata_file}")
        except Exception as e:
    ...
    def run(self):
        ...
        try:
            ...
            # Clean exit if setup failed. (i.e. lock file exists)
            return
        except Exception as e:
        ...
        try:
            ...
            self.event_handler = SBH.NewBamFileHandler(
                ...
                self.results_directory, self.gridion, self.shutdown_event, self.live_run)
